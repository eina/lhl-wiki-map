const { createNewPoint, getPointsByMapID } = require("./points");
...
module.exports = {
  ...
  getMapByID: (db, { mapID }) => {
    ...
    return db
      ...
      .then(pointData => {
        mapData = { ...mapData, points: pointData };
        return mapData;
      })
      .catch(err => {
  ...
  getMapsEditedByUser: (db, { userID }) => {
    ...
    let queryString = `
      ...
      FROM
        users
        FULL JOIN edits ON users.id = u_id
  ...
  createNewMap: (db, { userID, mapTitle, creationTime, centerLat, centerLng, points }) => {
    ...
    return db
      ...
      .then(() => {
        console.log("what are you", resultsJSON);
        return resultsJSON;
      })
      .catch(err => {
        return { status: 500, error: err.message };
  ...
  updateMap: (db, { mapID, mapData }) => {
    ...
    queryParams.push(mapData.mapTitle, mapData.centerLat, mapData.centerLng);
    queryString += `SET
      title = $1,
      center_lat = $2,
      center_lng = $3 `;
